...
export const LearnFromMistakes: React.FC<Props> = ({
  ...
  if (state.showPlayerSelection) {
    return (
      <div className="flex h-full w-full flex-col border-glass-border text-white">
        <div className="flex items-center justify-between border-b border-glass-border px-4 py-2">
          ...
          <button
            onClick={onStop}
            title="Stop learning"
            className="flex items-center gap-1 rounded border border-glass-border bg-white/5 px-2 py-1 text-xxs text-white/70 transition duration-200 hover:border-white/20 hover:bg-white/10 hover:text-white"
          >
        ...
        {!state.showSolution && lastMoveResult !== 'correct' ? (
          <>
            {!isLastMistake && (
              <button
                onClick={onNext}
                className="flex flex-1 items-center justify-center gap-1.5 rounded border border-glass-border bg-glass px-3 py-2 text-sm font-medium text-white/75 transition duration-200 hover:bg-glass-strong"
              >
